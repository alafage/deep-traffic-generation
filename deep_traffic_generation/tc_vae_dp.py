...
class TCEncoder(nn.Module):
    def __init__(
        ...
        self.z_loc = nn.Linear(h_dims[-1] * seq_len, out_dim)
        self.z_log_var = nn.Linear(h_dims[-1] * seq_len, out_dim)
...
class TCDecoder(nn.Module):
    def __init__(
        ...
        self.decode_entry = nn.Linear(input_dim, h_dims[0] * seq_len)
